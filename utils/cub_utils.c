...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/17 11:51:51 by ihovhann          #+#    #+#             */
/*   Updated: 2021/06/17 11:52:03 by ihovhann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */#include "../cub3d.h"

void	attach_textures(t_norm_print_map *n, t_cub *cub, int *x, int *y)
{
	n->tex_y = (int)n->tex_pos & (TEXHEIGTH - 1);
	n->tex_pos += n->step;
	if (n->side == 0)
	{
		if (n->step_x > 0)
	...
	*y = s->draw_start_y;
	while (*y < s->draw_end_y)
	{
		...
		* 128 + s->sprite_height * 128;
		s->tex_y = ((s->d * TEXHEIGTH) / s->sprite_height) / 256;
